                ...
                if (USER_SETTING.FORCE_RESOURCE_VIA_MEDIA) {
                  ...
                  if (element_images && imgLink) {
                    $(
                      ...
                      `<a datetime="${publish_time}" data-needed="direct" data-path="${state.GL_postPath}" data-name="photo" data-type="jpg" data-globalIndex="${s}" href="javascript:;" href="" data-href="${imgLink}"><img width="100" src="${imgLink}" /><br/>- <span data-ih-locale="IMG">${_i18n("IMG")}</span> ${s} -</a>`,
                    );
                  }
                }
              }
      ...
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          try {
            ...
            if (obj.status === "fail") {
              alert(
                `getBlobMediaWithQueryID(): Request failed with API response:\n${obj.message}: ${obj.feedback_message}`,
              );
              logger(
                `Request failed with API response ${obj.message}: ${obj.feedback_message}`,
              );
              reject(response);
            } else {
